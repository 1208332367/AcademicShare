					...
					<block v-if="resourceListLength > 0">
						<view class="resource_list">
							<block v-for="(resource, index) in resourceList" :key="index">
								<block v-if="index < 4">
									<Resource :showScore="false" :cancelUpload="true" :item="resource" style="border-bottom: #D3D3D3 solid 2px;"></Resource>
								</block>
								<block v-else>
									<Resource :showScore="false" :cancelUpload="true" :item="resource" v-if="index < resourceListLength - 1" style="border-bottom: #D3D3D3 solid 2px;"></Resource>
									<Resource :showScore="false" :cancelUpload="true" :item="resource" v-if="index == resourceListLength - 1"></Resource>
								</block>
							</block>
						</view>
					</block>
		...
		methods: {
			getResourceList:function(){
				...
				try{
					...
					uni.request({
						url: App.globalData.rootUrl + 'user/getUploadResource',
						data: {
